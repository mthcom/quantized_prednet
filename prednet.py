    ...
    def build(self, input_shape):
        ...
        for l in range(self.nb_layers):
            for c in ['i1', 'f1', 'c1', 'o1']:
                ...
                self.conv_layers[c].append(Conv2D(3, 1, padding='same', activation=act, data_format=self.data_format))
            for c in ['i3', 'f3', 'c3', 'o3']:
                act = self.LSTM_activation if c == 'c' else self.LSTM_inner_activation
                self.conv_layers[c].append(Conv2D(3, 3, padding='same', activation=act, data_format=self.data_format))
            for c in ['i5', 'f5', 'c5', 'o5']:
                act = self.LSTM_activation if c == 'c' else self.LSTM_inner_activation
                self.conv_layers[c].append(Conv2D(3, 5, padding='same', activation=act, data_format=self.data_format))

            act = 'relu' if l == 0 else self.A_activation
            self.conv_layers['ahat'].append(Conv2D(3, 3, padding='same', activation=act, data_format=self.data_format))
        ...
        for c in sorted(self.conv_layers.keys()):
            for l in range(len(self.conv_layers[c])):
                ds_factor = 2 ** l
                if c == 'ahat':
                    nb_channels = 3*3#matin
                elif c == 'a':
                    nb_channels = 2 * 3
                else:
                    nb_channels = 3 * 2 + 3*3#matin
                    if l < self.nb_layers - 1:
